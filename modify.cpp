...
void editTodo(const string& filename, const string& id, const vector<string>& entry);    
void editTodo(const string& filename, const string& id, const vector<string>& entry);
vector<string> getUserInput(const vector<string>& keys);
...
int main()
{
    ...
    string idToEdit;
    vector<string> newDataEntry;

    cout << "Enter the ID you want to edit: ";
    getline(cin, idToEdit);

    cout << "Please provide the updated information:" << endl;
    newDataEntry = getUserInput({"ID", "Todo", "Status", "Category", "Due Date", "Remarks"});

    editTodo("data.csv", idToEdit, newDataEntry);
    return 0;
}
...
// Function to get user input for updating a todo entry
vector<string> getUserInput(const vector<string>& keys) {
    vector<string> newEntry;
...
// Function to edit a todo entry in the file
void editTodo(const string& filename, const string& id, const vector<string>& entry) {
    fstream file(filename, ios::in | ios::out);
    if (!file.is_open()) {
        cerr << "Error: Cannot open file for editing." << endl;
        return;
    }
    ...
    while (getline(file, line)) {
        pos = line.find(id);
        if (pos != string::npos) {  
            file.seekp(static_cast<streamoff>(file.tellg()) - line.length() - 1);
            ...
            if (entry.size() < 7) {
                file << setw(7 - entry.size()) << setfill(' ') << "";
            }

            file << endl;
            cout << "Entry with ID " << id << " has been successfully updated." << endl;
            file.close();
            return;
        }
    }

    cout << "Error: Entry with ID " << id << " not found." << endl;
    file.close();
}
